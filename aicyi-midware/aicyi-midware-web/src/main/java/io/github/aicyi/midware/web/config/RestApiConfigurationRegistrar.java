...
/**
 * @author Mr.Min
 * @description {@link EnableRestApi} 装配注册器
 * <p>
 ...
 *     <li>{@link GlobalExceptionHandler}：全局异常处理器，由 {@code enableGlobalExceptionHandler} 控制</li>
 * </ul>
 * <p>
 * 重复标注 {@link EnableRestApi} 时仅首个声明生效，后续声明输出告警日志并跳过
 * @date 2026/8/14
 **/
public class RestApiConfigurationRegistrar implements ImportBeanDefinitionRegistrar {

    private static final Logger LOGGER = LoggerFactory.getLogger(RestApiConfigurationRegistrar.class);

    /**
     * 装配标记 Bean 定义名，用于检测重复标注
     */
    private static final String ENABLED_MARKER_BEAN_NAME = RestApiConfigurationRegistrar.class.getName() + ".enabledMarker";
    ...
    public void registerBeanDefinitions(@NonNull AnnotationMetadata importingClassMetadata, @NonNull BeanDefinitionRegistry registry) {
        ...
        Map<String, Object> attributes = importingClassMetadata.getAnnotationAttributes(EnableRestApi.class.getName());
        if (attributes == null) {
            return;
        }
        ...
        boolean enableGlobalExceptionHandler = annotationAttributes.getBoolean("enableGlobalExceptionHandler");
        String[] authExcludePathPatterns = annotationAttributes.getStringArray("authExcludePathPatterns");

        // Web MVC 配置：按拦截器 Bean 是否存在决定注册内容（始终注册）
        BeanDefinitionBuilder mvcConfigurationBuilder = BeanDefinitionBuilder.genericBeanDefinition(RestApiWebMvcConfiguration.class)
